...
ARG MAX_VERSION=26.3.0
ARG MOJO_VERSION=1.0.0b1
ARG CODE_BUILTIN_EXTENSIONS_DIR=/opt/code-server/lib/vscode/extensions
ARG QUARTO_VERSION=1.9.38
ARG CTAN_REPO=https://www.texlive.info/tlnet-archive/2026/06/18/tlnet

ARG INSTALL_MAX
ARG BASE_SELECT=${INSTALL_MAX:+max}

FROM ${BUILD_ON_IMAGE}${MAX_VERSION:+:$MAX_VERSION} AS base-max

ENV PARENT_IMAGE=${BUILD_ON_IMAGE}${MAX_VERSION:+:$MAX_VERSION}

FROM ${BUILD_ON_IMAGE}${MOJO_VERSION:+:$MOJO_VERSION} AS base-mojo
